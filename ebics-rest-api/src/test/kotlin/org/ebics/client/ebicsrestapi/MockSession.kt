...
class MockSession {
    companion object {
        fun getSession(userId: Long, bankCerts: Boolean = true, prod: Product, configuration: EbicsConfiguration): EbicsSession {
            val user = MockUser.createMockUser(userId, bankCerts)
            return getSession(user, prod, configuration, bankCerts)
        }

        fun getSession(user: User, prod: Product, configuration: EbicsConfiguration, bankCerts: Boolean = true): EbicsSession {
            return EbicsSession(
                user,
                configuration,
                prod,
