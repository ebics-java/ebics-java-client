...
class KeyManagementImpl(session: EbicsSession) : KeyManagement(session) {
    ...
    override fun sendINI(orderId: String?) {
        ...
        val request = INIRequestElement(session, orderId).apply { build(); validate() }
        ...
        response = KeyManagementResponseElement(responseBody, "INIResponse")
        response.build()
    ...
    override fun sendHIA(orderId: String?) {
        ...
        val sender = HttpRequestSender(session)
        val request = HIARequestElement(session, orderId).apply { build(); validate() }
        session.configuration.traceManager.trace(request, session)
        val responseBody = sender.send(ByteArrayContentFactory(request.prettyPrint()))
        response = KeyManagementResponseElement(responseBody, "HIAResponse")
        response.build()
        session.configuration.traceManager.trace(response, session)
        response.report()
    ...
    override fun sendHPB(passwordCallback: PasswordCallback): BankCertificateManager {
        val response: KeyManagementResponseElement
        val orderData: HPBResponseOrderDataElement
        ...
        session.configuration.traceManager.trace(request, session)
        val responseBody = sender.send(ByteArrayContentFactory(request.prettyPrint()))
        response = KeyManagementResponseElement(responseBody, "HBPResponse")
        response.build()
        session.configuration.traceManager.trace(response, session)
        response.report()
        factory =
            ByteArrayContentFactory(Utils.unzip(session.userCert.decrypt(response.orderData, response.transactionKey)))
        orderData = HPBResponseOrderDataElement(factory)
        orderData.build()
        session.configuration.traceManager.trace(orderData, session)
        val manager = if (session.user.useCertificate) createFromCertificates(
